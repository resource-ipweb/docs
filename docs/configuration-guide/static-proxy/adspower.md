---
sidebar_position: 5
sidebar_label: AdsPower 配置使用教程
description: 在 AdsPower 中配置 IPWeb 静态原生代理的完整教程。
---
# AdsPower 指纹浏览器配置 IPWeb 静态原生代理教程
...
其中，**静态原生代理**能够提供稳定的固定出口 IP，更适合对网络环境连续性和连接稳定性有要求的业务。本教程将介绍从注册、试用、购买静态原生代理，到在 **AdsPower 指纹浏览器**中完成 IPWeb 代理配置与连通性验证的完整流程。
...
在代理列表中，重点确认后续 AdsPower 配置需要使用的信息：
...
页面还会显示国家/地区、带宽、共享/独享、连接数（QPS）、到期/续费、UDP 等信息，可用于核对当前代理资源。后续配置 AdsPower 时，主要使用这里显示的 **代理 IP、端口号、账号和密码**。

如需了解代理筛选、账号导出、续费及其他管理操作，可参考 [IPWeb 静态原生代理使用指南](https://docs.ipweb.cc/static-native-user-guide/)。

## AdsPower 指纹浏览器配置

完成上述流程并获取 IPWeb 代理的服务器地址、端口、用户名和密码后，即可将代理添加到 AdsPower 浏览器环境中。

### 1、新建浏览器环境

打开 **AdsPower** 客户端，然后点击左上角的「**新建浏览器**」。

![AdsPower 新建浏览器环境](https://oss.ipweb.cc/docs/imgs/2026/09/adspower-image1.png)

### 2、填写 IPWeb 代理信息

进入新建浏览器页面后，找到代理信息区域，将「**代理方式**」选择为「**自定义**」，然后根据上一节从 IPWeb 后台获取的代理连接信息依次填写。

| AdsPower 配置项 | 填写说明 |
| --- | --- |
| **代理类型** | 选择与 IPWeb 当前代理对应的协议类型，例如 `HTTP`、`HTTPS` 或 `Socks5`。 |
| **IP 查询渠道** | 可保持 AdsPower 默认的 `IP2Location`，用于检测当前代理出口 IP 及位置信息。 |
| **主机：端口** | 填写 IPWeb「我的 IP」页面显示的代理 IP 地址和对应端口。 |
| **代理账号** | 填写 IPWeb 提供的代理账号。 |
| **代理密码** | 填写与代理账号对应的密码。 |
| **刷新 URL** | 如当前代理没有需要填写的刷新链接，可保持为空。 |
...
![AdsPower 填写 IPWeb 代理信息](https://oss.ipweb.cc/docs/imgs/2026/09/adspower-image2.png)

> **注意：** AdsPower 中选择的代理类型必须与 IPWeb 当前代理实际使用的协议一致。如果协议类型选择错误，即使服务器地址、端口和账密填写正确，也可能无法通过连接检测。

### 3、检查代理连接

代理信息填写完成后，点击「**检查代理**」。

![AdsPower 检查代理连接](https://oss.ipweb.cc/docs/imgs/2026/09/adspower-image3.png)

如果页面显示「**连接测试成功**」，并能够正常获取代理出口 IP、国家或地区、城市等信息，说明 IPWeb 代理已经成功连接到当前 AdsPower 浏览器环境。

此时可核对检测到的出口 IP 是否与 IPWeb 后台当前使用的代理信息一致。如果出口 IP 和目标地区符合预期，即可继续完成浏览器指纹、账号平台等其他环境配置。

### 4、保存并启动浏览器环境

代理检测通过后，点击页面底部的「**确定**」保存浏览器环境。

返回「**环境管理**」页面，找到刚刚创建的环境并打开。启动后，该浏览器环境的网络请求将通过已配置的 IPWeb 代理访问目标网站。

如需进一步确认代理是否生效，也可以在当前 AdsPower 浏览器环境中访问 `https://ipinfo.io` 或其他 IP 检测网站，检查当前显示的出口 IP 是否与 IPWeb 代理一致。
...
| 现象 | 建议检查项 |
| --- | --- |
| 无法连接代理 | 检查代理主机和端口是否填写正确，并确认当前 IPWeb 代理仍处于可用状态。 |
| 提示认证失败 | 重新核对 IPWeb 提供的代理账号和密码，避免复制时带入空格。 |
| 检测一直超时 | 检查本地网络是否可以正常连接代理服务器。 |
| 已填写账密但仍无法使用 | 确认 AdsPower 中选择的代理类型与 IPWeb 当前代理实际使用的协议一致。 |
| 浏览器打开后显示本地 IP | 返回当前浏览器环境的代理设置，确认代理已保存并通过「检查代理」。 |
| 检测地区与预期不一致 | 核对当前填写的 IPWeb 代理是否为所购买的目标国家 / 地区节点。 |

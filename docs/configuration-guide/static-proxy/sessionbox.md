---
sidebar_position: 19
sidebar_label: SessionBox 配置使用教程
description: 在 SessionBox 中配置 IPWeb 静态原生代理的完整教程。
---
# SessionBox 指纹浏览器配置 IPWeb 静态原生代理教程
...
其中，**静态原生代理**能够提供稳定的固定出口 IP，更适合对网络环境连续性和连接稳定性有要求的业务。本教程将介绍从注册、试用、购买静态原生代理，到在 **SessionBox 指纹浏览器**中完成 IPWeb 代理配置与连通性验证的完整流程。
...
在代理列表中，重点确认后续 SessionBox 配置需要使用的信息：
...
页面还会显示国家/地区、带宽、共享/独享、连接数（QPS）、到期/续费、UDP 等信息，可用于核对当前代理资源。后续配置 SessionBox 时，主要使用这里显示的 **代理 IP、端口号、账号和密码**。

如需了解代理筛选、账号导出、续费及其他管理操作，可参考 [IPWeb 静态原生代理使用指南](https://docs.ipweb.cc/static-native-user-guide/)。

## SessionBox 指纹浏览器配置

完成上述流程并获取 IPWeb 的服务器地址、端口、用户名和密码后，即可将代理添加到 SessionBox 浏览器配置文件中。

### 1、创建浏览器配置文件

打开 **SessionBox**，在「**My Profiles**」页面点击右上角的「**Create profile**」。

![SessionBox 创建配置文件](https://oss.ipweb.cc/docs/imgs/2026/09/sessionbox-1.png)
...
![SessionBox 进入代理管理](https://oss.ipweb.cc/docs/imgs/2026/09/sessionbox-2.png)

### 2、添加 IPWeb 代理

进入「**My proxies**」后，点击右上角的「**Add proxy**」。

![SessionBox 添加代理](https://oss.ipweb.cc/docs/imgs/2026/09/sessionbox-3.png)

在「**Create proxy**」页面中，将上一节从 IPWeb 后台获取的代理信息填写到对应位置：

| SessionBox 配置项 | 填写说明 |
| --- | --- |
| **Name** | 自定义代理名称，方便后续识别。 |
| **Type** | 选择与 IPWeb 当前代理协议一致的类型，例如 `HTTP`。 |
| **Address** | 填写 IPWeb 提供的代理主机地址。 |
| **Port** | 填写对应的代理端口。 |
| **Username** | 填写 IPWeb 提供的代理账号。 |
| **Password** | 填写对应的代理密码。 |
...
![SessionBox 填写 IPWeb 代理信息](https://oss.ipweb.cc/docs/imgs/2026/09/sessionbox-4.png)

> **注意：** SessionBox 中的代理类型必须与 IPWeb 获取代理时使用的协议一致。
...
如果测试失败，可以重点检查代理类型、Address、Port、Username 和 Password 是否与 IPWeb 后台提供的信息一致。

![SessionBox 测试 IPWeb 代理](https://oss.ipweb.cc/docs/imgs/2026/09/sessionbox-5.png)

### 4、绑定代理并启动配置文件

代理测试通过后，点击「**Back**」返回配置文件创建页面。

在「**Proxy**」下拉框中选择刚刚添加并测试成功的 IPWeb 代理，然后继续完成其他配置。

确认无误后，点击页面右上角的「**Create**」创建配置文件。

创建完成后，在「**My Profiles**」中启动对应配置文件，即可通过已配置的 IPWeb 代理访问网络。

如需进一步确认代理是否生效，也可以在当前 SessionBox 浏览器环境中访问 `https://ipinfo.io` 或其他 IP 检测网站，检查当前显示的出口 IP 是否与 IPWeb 代理一致。

### 5、常见问题排查

| 现象 | 建议检查项 |
| --- | --- |
| Test proxy 失败 | 检查代理类型、Address 和 Port 是否填写正确，并确认当前 IPWeb 代理仍然有效。 |
| 提示认证失败 | 重新核对 Username 和 Password，避免复制时带入空格。 |
| 测试一直失败 | 检查本地网络是否能够正常连接代理服务器。 |
| 浏览器启动后显示本地 IP | 确认已经在配置文件的 Proxy 下拉框中选中了测试通过的 IPWeb 代理。 |

---
sidebar_position: 14
sidebar_label: Lalicat 配置使用教程
description: 在 Lalicat 中配置 IPWeb 静态原生代理的完整教程。
---
# Lalicat 指纹浏览器配置 IPWeb 静态原生代理教程
...
其中，**静态原生代理**能够提供稳定的固定出口 IP，更适合对网络环境连续性和连接稳定性有要求的业务。本教程将介绍从注册、试用、购买静态原生代理，到在 **Lalicat 指纹浏览器**中完成 IPWeb 代理配置与连通性验证的完整流程。
...
在代理列表中，重点确认后续 Lalicat 配置需要使用的信息：
...
页面还会显示国家/地区、带宽、共享/独享、连接数（QPS）、到期/续费、UDP 等信息，可用于核对当前代理资源。后续配置 Lalicat 时，主要使用这里显示的 **代理 IP、端口号、账号和密码**。

如需了解代理筛选、账号导出、续费及其他管理操作，可参考 [IPWeb 静态原生代理使用指南](https://docs.ipweb.cc/static-native-user-guide/)。

## Lalicat 指纹浏览器配置

完成上述流程并获取 IPWeb 的服务器地址、端口、用户名和密码后，即可将代理添加到 Lalicat 浏览器配置中。

### 1、新建浏览器配置

打开 **Lalicat 指纹浏览器**，在左侧进入「**浏览器列表**」，点击页面上方的「**+ 添加浏览器**」。

![Lalicat 添加浏览器](https://oss.ipweb.cc/docs/imgs/2026/09/lalicat-1.png)

### 2、填写 IPWeb 代理信息

进入新建浏览器配置页面后，可以先填写显示名称、分组、浏览器版本、User-Agent 等基础信息，然后在「**基本配置**」中找到「**代理设置**」，选择与 IPWeb 当前代理一致的协议，例如 `HTTP`、`HTTPS` 或 `SOCKS5`。

随后依次填写：

| Lalicat 配置项 | 填写说明 |
| --- | --- |
| **IP地址** | 填写 IPWeb 提供的代理主机地址。 |
| **端口** | 填写对应的代理端口。 |
| **登录用户** | 填写 IPWeb 提供的代理账号。 |
| **登录密码** | 填写对应的代理密码。 |

如果已经复制了完整的代理信息，也可以使用右侧的「**粘贴代理信息**」功能快速录入。Lalicat 支持按页面提示的格式粘贴，若从 IPWeb 导出的账号格式为：
...
![Lalicat 填写 IPWeb 代理信息](https://oss.ipweb.cc/docs/imgs/2026/09/lalicat-2.png)

> **注意：** Lalicat 中选择的代理协议必须与 IPWeb 当前代理实际使用的协议一致。
...
如果检测没有通过，则会显示连接失败，可以优先检查代理协议、IP 地址和端口、登录用户和登录密码，以及当前 IPWeb 代理是否仍然有效。

![Lalicat 检查 IPWeb 代理连接](https://oss.ipweb.cc/docs/imgs/2026/09/lalicat-3.png)
...
保存后返回「**浏览器列表**」，找到刚刚创建的配置并启动，即可通过已配置的 IPWeb 代理访问网络。

如需进一步确认代理是否生效，也可以在当前 Lalicat 浏览器环境中访问 `https://ipinfo.io` 或其他 IP 检测网站，检查当前显示的出口 IP 是否与 IPWeb 代理一致。

### 5、常见问题排查

| 现象 | 建议检查项 |
| --- | --- |
| 无法连接代理 | 检查 IP 地址和端口是否填写完整，并确认当前 IPWeb 代理仍然有效。 |
| 提示认证失败 | 重新核对 IPWeb 提供的登录用户和登录密码，避免复制时带入空格。 |
| 检查网络失败 | 检查本地网络是否能够正常连接代理服务器。 |
| 已填写账密但仍无法使用 | 确认 Lalicat 中选择的代理协议与 IPWeb 实际代理协议一致。 |

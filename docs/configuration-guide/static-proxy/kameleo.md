---
sidebar_position: 13
sidebar_label: Kameleo 配置使用教程
description: 在 Kameleo 中配置 IPWeb 静态原生代理的完整教程。
---
# Kameleo 指纹浏览器配置 IPWeb 静态原生代理教程
...
其中，**静态原生代理**能够提供稳定的固定出口 IP，更适合对网络环境连续性和连接稳定性有要求的业务。本教程将介绍从注册、试用、购买静态原生代理，到在 **Kameleo 指纹浏览器**中完成 IPWeb 代理配置与连通性验证的完整流程。
...
在代理列表中，重点确认后续 Kameleo 配置需要使用的信息：
...
页面还会显示国家/地区、带宽、共享/独享、连接数（QPS）、到期/续费、UDP 等信息，可用于核对当前代理资源。后续配置 Kameleo 时，主要使用这里显示的 **代理 IP、端口号、账号和密码**。

如需了解代理筛选、账号导出、续费及其他管理操作，可参考 [IPWeb 静态原生代理使用指南](https://docs.ipweb.cc/static-native-user-guide/)。

## Kameleo 指纹浏览器配置

完成上述流程并获取 IPWeb 的**代理主机、代理端口、代理账号和代理密码**后，即可按照下面的步骤在 **Kameleo** 中完成配置。

### 1、新建浏览器配置文件

打开 Kameleo 客户端，在 **“Profiles”** 页面点击顶部的 **“New”**，新建一个浏览器配置文件。

![新建浏览器](https://oss.ipweb.cc/docs/imgs/2026/09/kameleo-1.png)

进入配置页面后，可先根据实际需求设置配置文件名称、设备类型、操作系统和浏览器等基础信息。

### 2、启用代理并填写 IPWeb 代理信息

在 **“Proxy”** 区域选择 **“With Proxy”**，然后根据 IPWeb 提供的代理信息完成配置。

![启用代理](https://oss.ipweb.cc/docs/imgs/2026/09/kameleo-2.png)

- **代理协议**：选择与 IPWeb 实际提供的代理协议一致的类型，例如 `HTTP` 或 `SOCKS5`。截图示例使用 `HTTP`。

- **IP or Hostname**：填写 IPWeb 提供的代理主机地址。
...
![填写 IPWeb 代理信息](https://oss.ipweb.cc/docs/imgs/2026/09/kameleo-3.png)

代理协议必须与 IPWeb 实际提供的协议保持一致。如果协议选择错误，即使代理主机、端口、账号和密码填写正确，也可能无法正常建立代理连接。
...
如果相关检测项目显示通过，并能够正常识别代理服务器位置，说明 IPWeb 代理已经可以正常使用。
...
- Kameleo 中选择的代理协议是否与 IPWeb 提供的代理协议一致；

- 当前代理是否仍然有效；

- 本地网络是否能够正常连接代理服务器。

![测试代理连接](https://oss.ipweb.cc/docs/imgs/2026/09/kameleo-4.png)
...
浏览器启动后，该配置文件的网络请求将通过已配置的 IPWeb 代理访问目标网站。

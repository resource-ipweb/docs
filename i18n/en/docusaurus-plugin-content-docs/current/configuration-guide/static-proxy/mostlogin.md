---
sidebar_position: 18
sidebar_label: MostLogin Configuration Guide
description: Complete guide to configuring IPWeb static native proxies in MostLogin.
---
# MostLogin + IPWeb Static Native Proxy Setup Guide
...
Among them, **Static Native Proxies** provide stable, fixed outbound IP addresses, making them better suited for use cases that require consistent network environments and reliable connections. This guide covers the complete process, from registration, free trial, and purchasing a Static Native Proxy to configuring and verifying an IPWeb proxy in the **MostLogin fingerprint browser**.
...
In the proxy list, pay particular attention to the following details, which will be used later when configuring MostLogin:
...
The page also displays information such as country/region, bandwidth, Shared/Dedicated status, connections (QPS), expiration/renewal, and UDP. These details can be used to verify the current proxy resource. When configuring MostLogin later, you will mainly use the **proxy IP, port, username, and password** shown here.
...
## Configure IPWeb Proxy in MostLogin

After completing the steps above and obtaining the IPWeb **proxy host, proxy port, proxy username, and proxy password**, you can configure the proxy in **MostLogin**.

### 1. Create a Browser Profile

Open the MostLogin client. On the **Profiles** page, click **Create profile**.

![Create a browser profile in MostLogin](https://oss.ipweb.cc/docs/imgs/2026/09/mostlogin1-en.png)

### 2. Enter the IPWeb Proxy Details

On the **Create profile** page, switch to the **Proxy** tab at the top, then select **Basic** under **Type**.

In the proxy settings area, enter the proxy details provided by IPWeb:

- **Proxy protocol**: Select the protocol that matches the current IPWeb proxy, such as `HTTP`, `HTTPS`, or `SOCKS5`. The example shown uses `HTTPS`.

- **Host:Port**: Enter the proxy server address and corresponding port provided by IPWeb.

- **Account**: Enter the proxy username provided by IPWeb.

- **Password**: Enter the corresponding proxy password.

- **Proxy rotate URL**: If the current IPWeb proxy provides a rotation or refresh URL, enter it here. Otherwise, leave this field blank.

![Enter IPWeb proxy details in MostLogin](https://oss.ipweb.cc/docs/imgs/2026/09/mostlogin2-en.png)

> The proxy protocol selected in MostLogin must match the protocol actually provided by IPWeb. If the wrong protocol is selected, the proxy connection may fail even when the host, port, username, and password are correct.

### 3. Check the Proxy Server IP

After entering the proxy information, click **Check proxy server IP**.

If the check succeeds, MostLogin will display the current proxy exit IP together with details such as country/region, state/province, city, longitude/latitude, time zone, and postal code. This indicates that the IPWeb proxy can connect successfully.

![Check the IPWeb proxy server IP in MostLogin](https://oss.ipweb.cc/docs/imgs/2026/09/mostlogin-3-en.png)

After the proxy check succeeds, continue with the remaining browser profile settings as needed.

After confirming the settings, click **Confirm** in the lower-right corner to save the profile.

Once the profile has been created, return to the profile list and launch the browser profile. After launch, network requests from that profile will access target websites through the configured IPWeb proxy.

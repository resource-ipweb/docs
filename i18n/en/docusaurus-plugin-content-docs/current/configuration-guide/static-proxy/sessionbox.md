---
sidebar_position: 19
sidebar_label: SessionBox Configuration Guide
description: Complete guide to configuring IPWeb static native proxies in SessionBox.
---
# SessionBox + IPWeb Static Native Proxy Setup Guide
...
Among them, **Static Native Proxies** provide stable, fixed outbound IP addresses, making them better suited for use cases that require consistent network environments and reliable connections. This guide covers the complete process, from registration, free trial, and purchasing a Static Native Proxy to configuring and verifying an IPWeb proxy in the **SessionBox fingerprint browser**.
...
In the proxy list, pay particular attention to the following details, which will be used later when configuring SessionBox:
...
The page also displays information such as country/region, bandwidth, Shared/Dedicated status, connections (QPS), expiration/renewal, and UDP. These details can be used to verify the current proxy resource. When configuring SessionBox later, you will mainly use the **proxy IP, port, username, and password** shown here.
...
## Configure IPWeb Proxy in SessionBox

After completing the steps above and obtaining the IPWeb server address, port, username, and password, you can add the proxy to a SessionBox browser profile.

### 1. Create a Browser Profile

Open **SessionBox**. On the **My Profiles** page, click **Create profile** in the upper-right corner.

![Create a browser profile in SessionBox](https://oss.ipweb.cc/docs/imgs/2026/09/sessionbox-1.png)

On the profile creation page, you can first enter the basic profile information, such as:

- **Name**: Enter a name for the profile.
- **Group**: Select a group if needed.
- **Color**: Choose an identifying color if needed.

In the **Proxy** field, if no proxy has been added yet, click **Manage proxies** in the lower-right corner to open the proxy management page.

![Open proxy management in SessionBox](https://oss.ipweb.cc/docs/imgs/2026/09/sessionbox-2.png)

### 2. Add an IPWeb Proxy

On the **My proxies** page, click **Add proxy** in the upper-right corner.

![Add a proxy in SessionBox](https://oss.ipweb.cc/docs/imgs/2026/09/sessionbox-3.png)

On the **Create proxy** page, enter the proxy information obtained from the IPWeb dashboard in the previous section:

| SessionBox Setting | What to Enter |
| --- | --- |
| **Name** | Enter a custom proxy name for easier identification. |
| **Type** | Select the protocol that matches the current IPWeb proxy, such as `HTTP`. |
| **Address** | Enter the proxy host address provided by IPWeb. |
...
![Enter IPWeb proxy details in SessionBox](https://oss.ipweb.cc/docs/imgs/2026/09/sessionbox-4.png)

> **Note:** The proxy type selected in SessionBox must match the protocol used by the IPWeb proxy.

### 3. Test the Proxy Connection

After the proxy is created, SessionBox returns to the **My proxies** list.

Find the proxy you just added, click the **⋮** menu on the right, then select **Test proxy**.

SessionBox will test the connection for the current proxy. If the test passes, the proxy information has been entered correctly and the proxy can be used for the browser profile.

If the test fails, first check whether the **Type**, **Address**, **Port**, **Username**, and **Password** match the information provided in the IPWeb dashboard.

![Test the IPWeb proxy in SessionBox](https://oss.ipweb.cc/docs/imgs/2026/09/sessionbox-5.png)

### 4. Assign the Proxy and Launch the Browser Profile

After the proxy test passes, click **Back** to return to the profile creation page.

From the **Proxy** drop-down menu, select the IPWeb proxy you just added and successfully tested, then complete the remaining profile settings.

After confirming the settings, click **Create** in the upper-right corner to create the profile.

Once the profile has been created, launch it from **My Profiles** to access the internet through the configured IPWeb proxy.

To further confirm that the proxy is working, you can also open `https://ipinfo.io` or another IP-checking website in the current SessionBox browser profile and verify that the displayed exit IP matches the IPWeb proxy.

### 5. Troubleshooting

| Issue | What to Check |
| --- | --- |
| **Test proxy** fails | Check that the proxy **Type**, **Address**, and **Port** are entered correctly, and make sure the current IPWeb proxy is still active. |
| Authentication failed | Recheck the **Username** and **Password**, and make sure no extra spaces were included when copying them. |
| The proxy test keeps failing | Check whether your local network can connect to the proxy server normally. |
| The browser shows the local IP after launch | Make sure the successfully tested IPWeb proxy is selected from the **Proxy** drop-down menu for the current profile. |
...
> The interface layout or button names may vary slightly between SessionBox versions. Refer to the interface shown in your current client. Use proxies only for lawful and compliant business purposes, and comply with the target website's Terms of Service and all applicable local laws and regulations.

---
sidebar_position: 7
sidebar_label: Incogniton Configuration Guide
description: Complete guide to configuring IPWeb static native proxies in Incogniton.
---
# Incogniton + IPWeb Static Native Proxy Setup Guide
...
Among them, **Static Native Proxies** provide stable, fixed outbound IP addresses, making them better suited for use cases that require consistent network environments and reliable connections. This guide covers the complete process, from registration, free trial, and purchasing a Static Native Proxy to configuring and verifying an IPWeb proxy in the **Incogniton fingerprint browser**.
...
In the proxy list, pay particular attention to the following details, which will be used later when configuring Incogniton:
...
The page also displays information such as country/region, bandwidth, Shared/Dedicated status, connections (QPS), expiration/renewal, and UDP. These details can be used to verify the current proxy resource. When configuring Incogniton later, you will mainly use the **proxy IP, port, username, and password** shown here.
...
## Configure IPWeb Proxy in Incogniton

After completing the steps above and obtaining the IPWeb proxy server address, port, username, and password, you can add the proxy to an Incogniton browser profile.

### 1. Create a New Browser Profile

Open the **Incogniton** client, go to **Profiles** from the left-hand menu, then click **Create profile** on the page.

![Create a new browser profile in Incogniton](https://oss.ipweb.cc/docs/imgs/2026/09/incogniton-image1-en.png)

### 2. Enter the IPWeb Proxy Details

On the profile creation page, open the **Proxy** tab and select **Custom proxy**, then enter the proxy connection details obtained from the IPWeb dashboard in the previous section.

| Incogniton Setting | What to Enter |
| --- | --- |
| **Connection type** | Select the protocol that matches the current IPWeb proxy. The example shown uses `HTTPS`. |
| **Proxy (ip:port)** | Enter the proxy host and port provided by IPWeb in the format `proxy-host:port`. |
...
Enter them in Incogniton as follows:

```text
IP:PORT   → Incogniton "Proxy (ip:port)"
USERNAME  → Incogniton "Proxy username"
PASSWORD  → Incogniton "Proxy password"
```

![Enter IPWeb proxy details in Incogniton](https://oss.ipweb.cc/docs/imgs/2026/09/incogniton-image2-en.png)

> **Note:** The **Connection type** must match the proxy protocol actually provided by IPWeb. If the wrong protocol is selected, the connection check may fail even when the server address, port, username, and password are correct.

### 3. Check the Proxy Connection

After entering the proxy information, click **Check** near the bottom of the page.

If the result shows **Connected** and displays the proxy exit IP, country/region, time zone, and other information correctly, the IPWeb proxy has been successfully connected to the current Incogniton browser profile.

![Check the IPWeb proxy connection in Incogniton](https://oss.ipweb.cc/docs/imgs/2026/09/incogniton-image3-en.png)

You can then verify that the detected exit IP matches the proxy information currently shown in the IPWeb dashboard. If the exit IP and target location match your expectations, continue with the remaining **Privacy & Network**, **Browser & Navigator**, **Hardware**, **Extensions & Cookies**, and other profile settings.

### 4. Save and Launch the Browser Profile

After confirming that the proxy connection check is successful, click **Create** in the lower-right corner to save the profile.

Once the profile has been created, launch the browser environment from the Incogniton profile list. After launch, network requests from that environment will access target websites through the configured IPWeb proxy.

To further confirm that the proxy is working, you can also open `https://ipinfo.io` or another IP-checking website in the current Incogniton browser environment and verify that the displayed exit IP matches the IPWeb proxy.

### 5. Troubleshooting

If the proxy check fails in Incogniton, check the following:
...
| Credentials are entered but the proxy still does not work | Make sure the **Connection type** selected in Incogniton matches the actual IPWeb proxy protocol. |
| The browser shows the local IP after launch | Return to the proxy settings for the current profile and confirm that the custom proxy has been saved and passed the connection check. |
...
> The interface layout or button names may vary slightly between Incogniton versions. Refer to the interface shown in your current client. Use proxies only for lawful and compliant business purposes, and comply with the target website's Terms of Service and all applicable local laws and regulations.

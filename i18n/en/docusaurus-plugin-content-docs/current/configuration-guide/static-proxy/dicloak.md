---
sidebar_position: 8
sidebar_label: DICloak Configuration Guide
description: Complete guide to configuring IPWeb static native proxies in DICloak.
---
# DICloak + IPWeb Static Native Proxy Setup Guide
...
In the proxy list, pay particular attention to the following details, which will be used later when configuring DICloak:
...
The page also displays information such as country/region, bandwidth, Shared/Dedicated status, connections (QPS), expiration/renewal, and UDP. These details can be used to verify the current proxy resource. When configuring DICloak later, you will mainly use the **proxy IP, port, username, and password** shown here.
...
## Configure IPWeb Proxy in DICloak

After completing the steps above and obtaining the IPWeb **proxy IP address, port, username, and password**, you can configure the proxy in **DICloak**.

### 1. Create a Browser Profile

Open the DICloak client, go to **Profiles** from the left-hand menu, then click **Create profile** at the top of the page.

![Create a browser profile in DICloak](https://oss.ipweb.cc/docs/imgs/2026/09/dicloak-1-en.png)

### 2. Enter the IPWeb Proxy Details

On the profile creation page, select **Custom Proxy** under **Proxy settings**.

If you already have the complete proxy information, you can use the **Quick input** field. Paste the proxy details and click **Analyze** to let DICloak automatically identify and populate the relevant fields.

![Enter IPWeb proxy details in DICloak](https://oss.ipweb.cc/docs/imgs/2026/09/dicloak-2-en.png)

You can also enter the information manually using the proxy details provided by IPWeb:

- **Proxy type**: Select the protocol that matches the proxy provided by IPWeb, such as `HTTP`, `HTTPS`, or `SOCKS5`.

- **IP protocol**: Select `IPv4` or `IPv6` based on the actual proxy configuration.

- **Host:Port**: Enter the proxy host address and corresponding port provided by IPWeb.

- **Proxy account**: Enter the proxy username provided by IPWeb.

- **Proxy password**: Enter the corresponding proxy password.

- **IP lookup channel**: You can keep DICloak's default setting to detect the proxy exit IP and related location information.

### 3. Check the Proxy Connection

After entering the proxy information, click **Checking proxy**.

If the page displays **Connection test succeeded** and correctly shows the proxy exit IP, country/region, state/province, city, and time zone, the IPWeb proxy has been configured successfully.

![Check the IPWeb proxy connection in DICloak](https://oss.ipweb.cc/docs/imgs/2026/09/dicloak-3-en.png)

After the proxy connection test succeeds, you can continue configuring other profile settings such as browser fingerprint, language, time zone, and geolocation.

After confirming the settings, click **Confirm** in the lower-right corner to save the browser profile.

Once the profile has been created, return to the **Profiles** page and launch it. After launch, network requests from that browser profile will access target websites through the configured IPWeb proxy.

If the connection test fails, check the following:

- Make sure the proxy host and port are entered correctly;

- Make sure the proxy account and password are correct;

- Make sure the proxy type selected in DICloak matches the proxy protocol provided by IPWeb;

- Make sure the IPv4/IPv6 option matches the actual proxy configuration;

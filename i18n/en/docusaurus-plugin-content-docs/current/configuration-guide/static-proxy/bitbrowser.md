---
sidebar_position: 11
sidebar_label: BitBrowser Configuration Guide
description: Complete guide to configuring IPWeb static native proxies in BitBrowser.
---
# BitBrowser + IPWeb Static Native Proxy Setup Guide
...
Among them, **Static Native Proxies** provide stable, fixed outbound IP addresses, making them better suited for use cases that require consistent network environments and reliable connections. This guide covers the complete process, from registration, free trial, and purchasing a Static Native Proxy to configuring and verifying an IPWeb proxy in the **BitBrowser fingerprint browser**.
...
In the proxy list, pay particular attention to the following details, which will be used later when configuring BitBrowser:
...
The page also displays information such as country/region, bandwidth, Shared/Dedicated status, connections (QPS), expiration/renewal, and UDP. These details can be used to verify the current proxy resource. When configuring BitBrowser later, you will mainly use the **proxy IP, port, username, and password** shown here.
...
## Configure IPWeb Proxy in BitBrowser

After completing the steps above and obtaining the IPWeb proxy server address, port, username, and password, you can add the proxy to a BitBrowser browser profile.

### 1. Create a Browser Profile

Open **BitBrowser**, select **Browser Profiles** from the left-hand menu, then click **Add** in the upper-left corner.

![Create a browser profile in BitBrowser](https://oss.ipweb.cc/docs/imgs/2026/09/bit-image1-en.png)

### 2. Enter the IPWeb Proxy Details

On the **Add** page, locate the **Proxy** section and set **Method** to **Custom**.

Using the proxy connection information obtained from the IPWeb dashboard in the previous section, complete the following fields:

| BitBrowser Setting | What to Enter |
| --- | --- |
| **Type** | Select the protocol that matches the current IPWeb proxy, such as `HTTP`. If you are using a SOCKS5 proxy, select the corresponding SOCKS5 option. |
| **Protocol** | Select `IPv4` or `IPv6` based on the current proxy configuration. |
...
![Enter IPWeb proxy details in BitBrowser](https://oss.ipweb.cc/docs/imgs/2026/09/bit-image2-en.png)

Make sure the proxy host, port, username, and password exactly match the information provided in the IPWeb dashboard. Incorrect information may cause the proxy check to fail or prevent the browser from connecting to the internet.

> **Note:** The **Type** and **Protocol** selected in BitBrowser must match the actual configuration of the current IPWeb proxy.

### 3. Check the Proxy Connection

After entering the proxy information, click **Check Proxy** on the right.

If the check succeeds, BitBrowser will display the current proxy exit IP together with details such as the country/region, state/province, city, and time zone. This indicates that the IPWeb proxy can connect successfully.

![Check the IPWeb proxy connection in BitBrowser](https://oss.ipweb.cc/docs/imgs/2026/09/bit-image3-en.png)
...
After the proxy check passes, complete the remaining browser profile settings as needed, then click **Confirm** in the lower-right corner to save the profile.

Once the profile has been created, launch it. After launch, network requests from that browser profile will access target websites through the configured IPWeb proxy server.

To further confirm that the proxy is working, you can also open `https://ipinfo.io` or another IP-checking website in the current BitBrowser profile and verify that the displayed exit IP matches the IPWeb proxy.

### 5. Troubleshooting

If the proxy check fails in BitBrowser, check the following:
...
| Credentials are entered but the proxy still does not work | Make sure the proxy protocol selected in BitBrowser matches the proxy type provided by IPWeb. |
| The browser shows the local IP after launch | Return to the proxy settings for the current browser profile and confirm that the custom proxy has been saved and passed **Check Proxy**. |
...
> The interface layout or button names may vary slightly between BitBrowser versions. Refer to the interface shown in your current client. Use proxies only for lawful and compliant business purposes, and comply with the target website's Terms of Service and all applicable local laws and regulations.

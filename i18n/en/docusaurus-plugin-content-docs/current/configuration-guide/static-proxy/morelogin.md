---
sidebar_position: 17
sidebar_label: MoreLogin Configuration Guide
description: Complete guide to configuring IPWeb static native proxies in MoreLogin.
---
# MoreLogin + IPWeb Static Native Proxy Setup Guide
...
In the proxy list, pay particular attention to the following details, which will be used later when configuring MoreLogin:
...
The page also displays information such as country/region, bandwidth, Shared/Dedicated status, connections (QPS), expiration/renewal, and UDP. These details can be used to verify the current proxy resource. When configuring MoreLogin later, you will mainly use the **proxy IP, port, username, and password** shown here.
...
## Configure IPWeb Proxy in MoreLogin

After completing the steps above and obtaining the IPWeb server address, port, username, and password, you can add the proxy to a MoreLogin browser profile.

### 1. Open the Profile and Access Proxy Settings

Open **MoreLogin**, go to **Profiles** from the left-hand menu, then switch to the **Browser** list.

![MoreLogin browser profile list](https://oss.ipweb.cc/docs/imgs/2026/09/morelogin-1-en.png)

Find the browser profile you want to configure. In the **Proxy** column, click the edit icon to open the **Modify Proxy** window.

If you do not have an available profile yet, click **New profile** in the upper-right corner to create a browser profile before configuring the proxy.

### 2. Add an IPWeb Proxy

In the **Modify Proxy** window, select **New proxy**.

![Add a new proxy in MoreLogin](https://oss.ipweb.cc/docs/imgs/2026/09/morelogin-2-en.png)

Then enter the proxy connection details obtained from the IPWeb dashboard in the previous section:
...
| **Proxy server** | Enter the proxy host and port provided by IPWeb. |
| **Proxy account** | Enter the proxy username provided by IPWeb. |
| **Proxy password** | Enter the corresponding proxy password. |
| **Refresh URL** | If the current proxy provides a refresh URL, enter it here. Otherwise, leave this field blank. |
...
```text
IP:PORT   → MoreLogin "Proxy server"
USERNAME  → MoreLogin "Proxy account"
PASSWORD  → MoreLogin "Proxy password"
```

![Enter IPWeb proxy details in MoreLogin](https://oss.ipweb.cc/docs/imgs/2026/09/morelogin-3-en.png)

> **Note:** The proxy type selected in MoreLogin must match the protocol used by the IPWeb proxy. Otherwise, the proxy may fail to connect.

### 3. Check the Proxy Connection

After entering the proxy information, click **proxy detection** at the bottom of the window.

If the check succeeds, MoreLogin will display **Check passed!** together with the current proxy exit IP and location information.

![Check the IPWeb proxy connection in MoreLogin](https://oss.ipweb.cc/docs/imgs/2026/09/morelogin-4-en.png)

After confirming that the proxy check passes, click **Confirm** in the lower-right corner to save the proxy settings.

### 4. Save and Launch the Browser Profile

After the proxy settings are saved, return to the MoreLogin profile list.

![MoreLogin profile list after saving the proxy](https://oss.ipweb.cc/docs/imgs/2026/09/morelogin-5-en.png)

The **Proxy** column will now show the proxy information and location assigned to the current profile.

Click **Start** for the profile to launch the browser. Once started, the profile will connect to the internet through the configured IPWeb proxy.

To further confirm that the proxy is working, you can also open `https://ipinfo.io` or another IP-checking website in the current MoreLogin browser profile and verify that the displayed exit IP matches the IPWeb proxy.

### 5. Troubleshooting

| Issue | What to Check |
| --- | --- |
| Unable to connect to the proxy | Check that the proxy server address and port are correct, and make sure the current IPWeb proxy is still active. |
| Authentication failed | Recheck the proxy account and password provided by IPWeb, and make sure no extra spaces were included when copying them. |
| Proxy check fails | Check whether your local network can connect to the proxy server normally. |
| Credentials are entered but the proxy still does not work | Make sure the proxy type selected in MoreLogin matches the actual IPWeb proxy protocol. |
| The browser shows the local IP after launch | Return to the proxy settings and confirm that the proxy has been saved and assigned to the current browser profile. |
...
> The interface layout or button names may vary slightly between MoreLogin versions. Refer to the interface shown in your current client. Use proxies only for lawful and compliant business purposes, and comply with the target website's Terms of Service and all applicable local laws and regulations.
